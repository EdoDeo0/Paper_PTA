...
cat("separatamente su campioni di dimensione diversa, quindi i due numeri non sono confrontabili\n")
cat("sulla stessa scala. Usare il love plot (differenze di media, non basato su istogrammi).\n\n")
cat("RI-ESEGUITO 2026-07-03/2026-06-26 dopo il fix C1 (audit): pre_lnvalue/pre_unitvalue usavano\n")
cat("erroneamente ln_export_value (unit value, non valore) per ENTRAMBE le covariate. Con le\n")
cat("covariate corrette il quadro CAMBIA rispetto alla nota precedente (non piu' valida):\n")
cat("  - pre_lnvalue   (valore export):     BEN bilanciata dopo match (SMD ~0.02, sotto soglia 0.1)\n")
cat("  - pre_unitvalue (prezzo unitario):   bilanciata dopo match (SMD ~0.085, sotto soglia 0.1)\n")
cat("  - pre_hhi       (concentrazione):    NON risolta — SMD ~0.18 dopo match (soglia 0.1), solo\n")
cat("                                        lieve miglioramento vs ~0.21 pre-match\n")
cat("Quindi SOLO 2 covariate su 3 sono ben bilanciate dal match; la concentrazione di mercato resta\n")
cat("uno squilibrio residuo da riportare esplicitamente come limite di C-prod-match nel paper.\n")
