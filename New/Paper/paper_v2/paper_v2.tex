...
economies (23 after excluding the entrepôt economies of Hong Kong and Macao;
Table~\ref{tab:treatment}). Three features of this treatment landscape
...
A \emph{bounded null} is a null result whose magnitude is bounded from above by
bootstrap confidence intervals, ruling out effects above a specific threshold.
...
Table~\ref{tab:brandi} places these estimates alongside those of \citet{brandi2020}.
...
the standard errors from the Stata implementation of \citet{sun2021} --- which, unlike
the R implementation, accounts for the uncertainty in the estimated cohort shares used
as weights\footnote{Specifically, \texttt{eventstudyinteract} in Stata estimates the
cohort shares, while \texttt{fixest::sunab} in R treats them as known weights. See
Appendix~\ref{app:sunab}.} --- no coefficient in the $[-10, +8]$ window is individually distinguishable
from zero on either margin. The dirty coefficient at $t = -6$ is marginally significant,
but a leave-one-cohort-out exercise shows it is driven by a single cohort and it
disappears when the window is truncated to $[-5, +5]$. Two or three nominally significant
coefficients among 28 estimates are what sampling noise produces under flat pre-trends
with 23 treated clusters. The ATT is null across all three specification variants: $p = 0.28$ in
...
at each destination's maximum (post-entry) value. Source: authors' elaboration on WB and TREND data.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
HS6--destination--year grid (8.2 million cells, 7.9 million after singleton removal), PPML estimates confirm the null:
...
The findings are specific to China's agreements of this period. Whether they extend to
other countries' PTAs or to China's post-2015 agreements --- which contain richer
environmental content --- remains an open question.
