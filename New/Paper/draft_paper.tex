...
both margins. The green interaction is stable across six estimation designs (full panel,
collapsed panel, and four control-group subsamples), and survives wild cluster bootstrap
...
bootstrap p-value is 0.18. The central reading is not that environmental provisions are
...
0.18, permutation inference on the estimating equation gives $p=0.08$ (and flips its
sign at the aggregate level), the TREND index never
...
(17 deep vs.\ 6 shallow in-sample) & 5.3M & the ``any agreement'' contrast
itself \\
\bottomrule
\end{tabular}
\begin{tablenotes}\footnotesize
...
of shallow clusters (6), which the few-cluster inference battery of
...
saturated structure is confirmed by a wild cluster bootstrap (B=9{,}999; $p=0.91$ for
the WB index, $0.64$ for TREND, and $0.89$/$0.62$ with controls). The pattern ---
...
sample, WB EP depth and non-environmental TotalDepth correlate at $0.88$ in the raw data
and $0.95$ after destination and year demeaning (the variation the design actually
uses); the raw-data VIF of $4.4$ is comparable to the maximum of $4.6$ that
...
cluster bootstrap p-value is 0.88, and permutation inference on the exact estimating
equation --- 1{,}000 reassignments of entire EP profiles across treated destinations
--- gives $p=0.90$: the observed coefficient is more central in the placebo
distribution than nine draws out of ten.
...
is $[-0.0088, +0.0046]$ per WB provision (collapsed panel: $[-0.0146, +0.0100]$): the
upper bound is about one thirty-fifth of the Brandi-equivalent effect, so the design
...
full-panel point estimate at face value ($-0.0040$ per provision), the response is an
...
treated destination--years ($\approx$ 3 provisions) moves green relative to neutral
exports by at most $-2.7\%$ to $+1.4\%$ at the 95\% bounds.
...
\quad permutation $p$ & 0.90 & 0.079 & 0.17 & 0.85 \\
\bottomrule
\end{tabular}
\begin{tablenotes}\footnotesize
...
aggregate gives $p=0.45$ for green and reverses the dirty sign ($+0.004$, $p=0.50$).
\sym{*} $p<0.05$ (asymptotic only; see text).
\end{tablenotes}
\end{threeparttable}
\end{table}
...
cluster bootstrap raises its p-value to 0.18. Permutation inference on the exact
estimating equation raises it to 0.079; a coarser permutation on a
destination--year--product-type aggregate goes further and reverses its sign
(+0.004, $p=0.50$), because aggregating to one observation per
destination--year--product-type collapses within-cell heterogeneity that the
disaggregated coefficient exploits. And a
...
counterpart: if destinations signing deep environmental chapters have green demand
rising for reasons of their own, allowing each destination its own linear trend in the
green (and dirty) gap should matter. Two variants of this exercise, run on the
...
results are unchanged (EP$\times$green $-0.0054$, bootstrap $p=0.28$; EP$\times$dirty
$-0.0070$, bootstrap $p=0.28$), but the TREND green interaction flips sign relative to
the baseline (from $+0.0018$, $p=0.32$, to $-0.0022$) and --- uniquely in this paper
--- survives the wild cluster bootstrap ($p=0.013$). Taken at face value it would say
...
WB$\times$green $+0.0176$, bootstrap $p=0.46$; dirty interactions $p=0.30$--$0.61$).
...
the same reading as the rest of the paper: EP$\times$intensity is $-0.0033$ with the
WB index (bootstrap $p=0.52$) and $-0.0013$ with TREND (asymptotic $p=0.012$,
