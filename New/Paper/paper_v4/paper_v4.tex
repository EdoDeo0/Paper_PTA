...
\addlinespace
\textbf{C-overlap}\newline \emph{\small products exported both to treated and untreated destinations} & Extrapolation outside common support & 21.5M \\
\addlinespace
...
of environmental content from the effect of entry timing. The non-environmental depth control is
permuted together with the environmental profile, so the null hypothesis is that, given which
destinations have an agreement and when, it does not matter which agreement profile --- environmental
and non-environmental content alike --- each of them received.\footnote{Estimates on the collapsed and destination-level panels were produced twice, in \textsc{Stata}
...
non-environmental depth and $0.891$ with DESTA depth.\footnote{These are partial correlations computed
on the residualized regressors, and they are therefore higher than the raw correlation of
$\rho = 0.91$ across treated destination--years reported in Section~\ref{sec:strategy}. The fixed
effects remove variation common to the two depth measures, which tightens what remains.} The World
...
neutral exports for a one-standard-deviation increase in environmental content. The corresponding
...
\input{Tabelle/tab_02_fullpanel}
...
at least as large in absolute value as the observed one. It is computationally expensive, which is
...
For the dirty coefficient the answer is that it would, 27.8 percent of the time. For green it is 59.7
...
estimate --- they supply the variation that identifies it, and without them the design cannot
...
\input{Tabelle/tab_03_collapsed}

\input{Tabelle/tab_04_leaveoneout}
...
\input{Tabelle/tab_05_subsamples}
...
The dilution is severe, and Table~\ref{tab:mechanism} quantifies it. The World Bank index checks 150
provision slots across the treated destinations, and 8 of them --- 5.3 percent --- fall in
...
\end{tabular}
\begin{tablenotes}\footnotesize
\item \emph{Notes:} Panel~A sums each depth index across the 25 treated destinations at each destination's maximum (post-entry) value. ``Checked'' is the total number of provision slots coded; ``Trade-mechanism'' counts those in sub-indices with a direct channel to trade flows. Panel~B counts Chinese PTAs in the TREND database; binding obligations are the TREND variable \texttt{X5.01.01 Binding obligations}, coded 1 when the environmental chapter states an obligation in binding terms, and the single agreement carrying it is the China--Korea PTA (2015). Source: World Bank DTA Database; TREND Database.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
by the same amount, so the green--dirty differential is $+0.0017$ with $p = 0.80$: this is not a shift
...
destination--years and correlates 0.87 with the aggregate TREND count and 0.90 with World Bank
non-environmental depth, which makes it a poor placebo rather than a
...
\input{Tabelle/tab_06_subindex_fullpanel}
...
force, with never-treated destinations as controls. The pre-treatment coefficients are flat on both
margins, which is the standard falsification of the identifying assumption. It should be read as
...
interaction significantly negative, the only green coefficient in the robustness battery to
survive the bootstrap. Estimating the same trends on pre-treatment years only and projecting them
...
quarter of the reference estimate in the aggregate literature. This is not a failure to reject.
...
\input{Tabelle/tab_A01_trattamento}
...
\input{Tabelle/tab_A03_permutation}
...
\input{Tabelle/tab_A05_ladder_wb}

\input{Tabelle/tab_A06_ladder_trend}
...
identified by the earliest cohorts alone.

\input{Tabelle/tab_A07_eventstudy_twfe}
...
The only green coefficient in the robustness battery that survives the wild cluster bootstrap
...
noise. More directly, Panel~B$'$ shows that the green gap was already rising before entry into force
...
\input{Tabelle/tab_A09_desttrends}
...
\input{Tabelle/tab_A10_subindex_collapsed}
...
\input{Tabelle/tab_A11_fullpanel_inclHKMO}

\input{Tabelle/tab_A12_collapsed_inclHKMO}
...
\input{Tabelle/tab_A13_robustness_full}
...
The distinction it draws is worth stating explicitly. A null result can mean that the effect is absent or that the design cannot see it, and only an interval separates the two. On the green margin the bootstrap interval rules out effects larger than roughly three percent per provision at 95 percent confidence, and says nothing about anything smaller. For scale, \citet{brandi2020} report an effect of 0.157 log points per trade-relevant provision on the green export share in their aggregate panel; the upper bound of 3.2 percent per provision here is roughly one quarter of that figure.

\input{Tabelle/tab_A16_mde}
