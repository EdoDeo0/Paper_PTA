************************************************************
*** 48k — Deep/Shallow subsample: coefficienti + WCB per val, qua, UV
*** Full panel con filtro deepshallow, FE fpd+fdt+pt
*** Supporta entrambi i depth (totaldepth / desta) via PTA_DEPTH
************************************************************
* OUTPUT: New/Output/TripleDiff/Tables_Stata/deepshallow_alldepvars{OUTSFX}.csv
...
cap log close _all
log using "$ROOT/New/Output/Diagnostics/stata_logs/48k_deepshallow_alldepvars$OUTSFX.log", replace text

global CSV_OUT "$ROOT/New/Output/TripleDiff/Tables_Stata/deepshallow_alldepvars$OUTSFX.csv"
...
************************************************************
* Programma: stima Deep/Shallow per una outcome
************************************************************
capture program drop ds_one_outcome
program define ds_one_outcome
    args raw_var outcome_label

    di as result _n "============================================================"
    di as result "  DeepShallow: `outcome_label' ($PTA_DEPTH)"
    di as result "============================================================"
    ...
    * --- Filtro Deep/Shallow ---
    ...
    drop keep_ds
    count
    di "  Righe dopo filtro DeepShallow: " r(N)
...
* Esecuzione
************************************************************
di as result _n "*** 48k — Deep/Shallow: val + quantity + unit value ***"
di as result "*** Depth: $PTA_DEPTH | Sample: $PTA_SAMPLE ***"

ds_one_outcome ln_export  ln_export
ds_one_outcome exp_qua    ln_export_qua
ds_one_outcome uv_exp     ln_export_value
