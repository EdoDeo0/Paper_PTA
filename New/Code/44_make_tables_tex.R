...
########################################################################
{
  ...
  L <- c("\\begin{table}[htbp]", "\\centering", "\\footnotesize",
    ...
    paste0("EP x green/dirty su outcome detrendizzato, Verde & ", b_wb_g$b, " \\\\"),
    paste0(" & ", b_wb_g$se, " \\\\"),
    paste0("\\quad $p$-value bootstrap & ", b_wb_g$pw, " \\\\"), "\\addlinespace",
    paste0("EP x green/dirty su outcome detrendizzato, Sporco & ", b_wb_b$b, " \\\\"),
    paste0(" & ", b_wb_b$se, " \\\\"),
    ...
    paste0("EP x green/dirty su outcome detrendizzato, Verde & ", b_tr_g$b, " \\\\"),
    paste0(" & ", b_tr_g$se, " \\\\"),
    paste0("\\quad $p$-value bootstrap & ", b_tr_g$pw, " \\\\"), "\\addlinespace",
    paste0("EP x green/dirty su outcome detrendizzato, Sporco & ", b_tr_b$b, " \\\\"),
    paste0(" & ", b_tr_b$se, " \\\\"),
...
########################################################################
{
  ...
  if (!is.null(base)) {
    ...
    L <- c(L, "\\bottomrule", "\\end{tabular}",
      ...
      "\\item \\textbf{Avvertenza sulle stelle.} Gli asterischi derivano dai $p$-value asintotici con cluster per destinazione prodotti dallo stimatore, non dal \\emph{wild cluster bootstrap} usato altrove. Con circa 23 destinazioni trattate quei $p$-value sono fortemente sottostimati: lo stesso coefficiente di riferimento della colonna (1) ha $p$ asintotico $<$0.001 ma $p$ bootstrap 0.070 (Tabella~\\ref{tab:wcb}). Le stelle di questa tabella non sono quindi confrontabili con quelle del resto del documento e non vanno lette come evidenza di significativit\\`a.",
