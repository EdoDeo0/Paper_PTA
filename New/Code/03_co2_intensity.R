...
##              25_robustness_co2intensity.R). Run: ~1 min.
##
## Cosa fa: costruisce una misura CONTINUA di intensita' CO2 per HS6, da
## affiancare al dirty_p binario (02), usando il replication package di
## Shapiro (QJE 2021), specifico per la CINA.
##
...
## binario in 02_dirty_goods.R. Crosswalk quindi al livello DIVISIONE (2
...
##         New/Data/Collapsed/panel_pdt_collapsed.fst (solo per il cross-check diagnostico)
...
COLLAPSED_FST <- here("New/Data/Collapsed/panel_pdt_collapsed.fst")
...
pop <- unique(as.data.table(read_fst(COLLAPSED_FST, columns = "hs6")))
