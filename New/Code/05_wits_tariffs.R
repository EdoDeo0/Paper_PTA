########################################################
###### 05 — Tariffe preferenziali WITS TRAINS (INCOMPLETO) ###
########################################################
## Author: Edoardo Vitella
## Sostituisce: 04_wits_pref_tariffs.R. Riscritto SOLO per documentazione,
##              NON rieseguito: l'API WITS-TRAINS era irraggiungibile
##              all'ultimo tentativo (verificato 2026-07-06, vedi sotto) e
##              nessun file e' mai stato scaricato (New/Data/WITS/raw/ e'
##              vuota). Il paper documenta questa limitazione esplicitamente
##              (draft_paper.tex, nota su "WITS-TRAINS API was unavailable
##              at the time of writing"): NON e' un input della pipeline
##              attuale, e' un tentativo di controllo mai completato.
##
## Cosa fa (se l'API tornasse disponibile): scarica da WITS (API SDMX, nessuna
## autenticazione) le tariffe applicate dalle ~25 destinazioni PTA verso la
## Cina, HS6, 2000-2015, per costruire un controllo di robustezza sulle
## tariffe preferenziali (tariffs_pref) da affiancare alla MFN gia' usata.
##
...
##
## STATO API (verificato 2026-07-06): l'API SDMX di WITS e' ROTTA lato server.
...
## o WITS "Tariff Download Facility".
##
## Due modalita' (argomento a riga di comando, default "download"):
...
##
## Output (mai raggiunto): New/Data/WITS/wits_pref_tariffs_hs6.csv
...
# tra cui Switzerland=141 collideva col vero codice del Vietnam
reporters <- data.table(
  iso3n = c("036", "050", "096", "116", "152", "188", "344", "352", "360", "356",
            "410", "418", "458", "446", "104", "554", "586", "608", "604", "702",
            "144", "756", "764", "626", "704"),
  name  = c("Australia", "Bangladesh", "Brunei", "Cambodia", "Chile", "CostaRica",
            "HongKong", "Iceland", "Indonesia", "India", "KoreaRep", "Laos",
            "Malaysia", "Macau", "Myanmar", "NewZealand", "Pakistan", "Philippines",
            "Peru", "Singapore", "SriLanka", "Switzerland", "Thailand",
            "TimorLeste", "Vietnam"),
  country_code = c(601L, 103L, 105L, 107L, 412L, 415L, 110L, 322L, 112L, 111L,
                    133L, 119L, 122L, 121L, 106L, 609L, 127L, 129L, 434L, 132L,
                    134L, 331L, 136L, 144L, 141L)
)
years <- 2000:2015
...
if (mode == "download") {
  ...
  log <- list()
  for (i in seq_len(nrow(reporters))) {
    for (y in years) {
...
if (mode == "parse") {
  ...
  dt <- rbindlist(lapply(files, parse_one), fill = TRUE)
